...
\item \textbf{Measurement} (\Cref{app:rfmatch}). A CUDA realisation measures $2.24$--$2.89\times$ in \texttt{fp32} and up to $3.85\times$ in \texttt{fp16} against the reference, and is faster than the dense $7\times7$ convolution WTConv is proposed to replace at every level. We report traffic and latency separately and do not claim a proportionality between them (\Cref{sec:prediction}).
...
We develop the argument for a single CUDA realisation to keep the exposition concrete. Because the reformulation is about data movement and not about any hardware feature, it transfers unchanged; we summarise Triton and Apple~Metal realisations in \Cref{app:backends}.
...
\caption{HBM dataflow for one WTConv decomposition level $\ell$, with $N_\ell = N/4^{\ell-1}$ the number of elements entering that level. (a)~The reference implementation expresses the level as a chain of framework primitives; every stage materialises a full-size tensor in HBM, so the level costs $6N_\ell$ of traffic. (b)~The fused formulation reads the input once, evaluates the Haar butterfly and the depthwise convolution on chip against weights that already carry the learned scale, and writes only the results that later stages genuinely need, for $\tfrac94 N_\ell$. $\tY$ and $\tZ$ are never written; the deepest level omits the raw low-pass write as well, costing $2N_L$.}
\label{fig:dataflow}
\end{figure}
...
Because WTConv operates at an arithmetic intensity of $0.74$~FLOP/byte (\Cref{sec:memorybound}), computing these coefficients on-the-fly inside the depthwise convolution kernel is essentially free in arithmetic while drastically reducing memory traffic. The subtlety is that the transform and the convolution have different access patterns: the Haar step consumes non-overlapping $2\times2$ blocks, whereas adjacent $k\times k$ convolution outputs share $k^2-1$ of their taps. Loading each tap's $2\times2$ block from HBM independently would read every input pixel $k^2$ times.

Our kernel therefore splits the work into spatial \emph{tiles} and stages the transform, not the input, on chip. Each thread block evaluates \Cref{eq:butterfly} once per $2\times2$ block over its output tile plus a halo of $R=(k-1)/2$ coefficient positions on each side, and holds the resulting four subband planes in shared memory; all $k^2$ taps of every output in the tile then read from that staged copy. The Haar transform is thus evaluated once per input block rather than once per tap, and each input pixel is fetched once per tile that needs it. Consequently, a level publishing its filtered subbands reads $N_\ell$ elements, writes $N_\ell$ convolved coefficients for synthesis, and writes $\tfrac14N_\ell$ raw low-pass coefficients for the subsequent level---requiring $\tfrac94N_\ell$ elements total, compared to $6N_\ell$ in the reference implementation. The deepest level $L$ feeds no successor and therefore moves only $2N_L$. We count halo positions once, as compulsory traffic: they are re-staged by each neighbouring tile, but those re-fetches are small, spatially local, and served by L2 rather than HBM, so $N_\ell$ is a lower bound on the read term and not an exact device-level count.
...
WTConv's difficulty was never its arithmetic. Expressed as a chain of framework primitives it runs at under $2\%$ of a modern GPU's arithmetic capability, spending its time moving tensors that need never have existed. Accounting for those bytes exactly, and removing them with three algebraic identities (a register-resident multiplication-free analysis, a synthesis cascade collapsed into a bit-indexed closed form, and a scale folded into the weights), yields a bit-exact reformulation whose cost is within a small constant of the compulsory traffic. We do not claim that the ratio of bytes predicts the ratio of seconds; what the account buys is the identification of the quantity to remove, and that the same identities reproduce the gain on two other stacks suggests the identification is the right one. The broader point is that an operator's asymptotic parameter or FLOP advantage says little about whether it is usable; for the growing class of layers built from cheap, structured, multi-stage transforms, the I/O cost model is the one that predicts practice.
...
Comparing every method at the same kernel size isolates the cost of the wavelet
machinery, but it is not the comparison a practitioner faces. WTConv is proposed
as a replacement for a \emph{large} convolution: an $L$-level layer at $k{=}3$
reaches $3\cdot2^{L}$ pixels, so even $L{=}2$ already exceeds the footprint of a
$7\times7$ kernel. \Cref{tab:rfmatch} therefore reports the measurement with the
plain-convolution baselines at $k{=}7$, all ratios taken against the reference
WTConv.
